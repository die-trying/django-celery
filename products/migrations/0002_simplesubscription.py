...
def createSimpleSubscription(apps, schema_editor):
    SimpleSubscription = apps.get_model('products.SimpleSubscription')
    OrdinaryLesson = apps.get_model('lessons.OrdinaryLesson')
    PairedLesson = apps.get_model('lessons.PairedLesson')
    LessonWithNative = apps.get_model('lessons.LessonWithNative')

    s = SimpleSubscription(
        name='Beginners subscription',
        internal_name='Beginners subscription',
        cost=150,
    )
    s.save()

    for i in range(1, 9):
        ordinary_lesson = OrdinaryLesson(
            name="Curated lesson %d/8" % i,
            internal_name="Curated lesson %d/8" % i,
        )
        ordinary_lesson.save()
        s.ordinary_lessons.add(ordinary_lesson)

    paired_lesson = PairedLesson(
        name="Paired lesson 1/1",
        internal_name="Paired lesson (beginners)",
    )
    paired_lesson.save()
    s.paired_lessons.add(paired_lesson)

    native_lesson = LessonWithNative(
        name="Lesson with native speaker 1/1",
        internal_name="Native speaker lesson (beginners)",
    )
    native_lesson.save()
    s.lessons_with_native.add(native_lesson)
    ...
    operations = [
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('paired_lessons', models.ManyToManyField(to='lessons.PairedLesson')),
            ],
            options={
                'abstract': False,
